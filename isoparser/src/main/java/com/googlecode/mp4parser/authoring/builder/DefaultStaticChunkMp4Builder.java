...
/**
 * 32bit版のMP4ファイルを作成するためのクラス(StaticChuckOffsetBoxをコンテナに使う)
 */
public class DefaultStaticChunkMp4Builder implements Mp4Builder {

    private static Logger LOG = Logger.getLogger(DefaultStaticChunkMp4Builder.class);
...
//        tkhd.setInPoster(true);
    ...
    int[] getChunkSizes(Track track) {
        ...
        assert DefaultStaticChunkMp4Builder.this.track2Sample.get(track).size() == sum(chunkSizes) : "The number of samples and the sum of all chunk lengths must be equal";
        return chunkSizes;
